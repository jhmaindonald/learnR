...
\marginnote[12pt]{CRAN is the primary R `repository'.  For preference,
obtain R and R packages from a CRAN mirror in the local region, e.g.,\\
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
remotes::\hlkwd{install_github}(\hlstr{'jhmaindonald/DAAGviz'},
<<<<<<< HEAD
                        build_opts=\hlkwd{c}(\hlstr{"--no-resave-data"}, \hlstr{"--no-manual"}))
=======
                        build_opts=\hlkwd{c}(\hlstr{"--no-resave-data"}, 
                                     \hlstr{"--no-manual"})
>>>>>>> e5442844fdb783da5761d94f5becf5cd061e2bd0
\end{alltt}
\end{kframe}
