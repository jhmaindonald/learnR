...
SPSS portable file '/var/folders/00/_kpyywm16hnbs2c0dvlf0mwr0000gq/T//RtmpCES8qI/file14ab64fc35b5c/NES1948.POR' 
...
\centerline{\includegraphics[width=0.97\textwidth]{figs/09-cline-gph-1} }
...
\centerline{\includegraphics[width=0.98\textwidth]{figs/09-my-panel-1} }
...
\end{Schunk}
\noindent
returns a model object.  When returned to the command line in this way,
a printed summary is returned.  Alternatively, as was done above, it
can be saved as a named object.  Here, what is returned is a list.
...
\margtt{plot(mftime.lm, chich=4}.  These give a different slant
...
\centerline{\includegraphics[width=0.47\textwidth]{figs/11-diag-logmf-1} }
...
\marginnote{The simulations assume that the model is correct.
  If the assumption of independent random errors is wrong,
...
\marginnote{A further issue, which use of separate test data does not
...
is:
 ...
 \marginnote{Removal of terms with $p > 0.15$ or $p > 0.2$
     ...
     that affect the interpretation of model results.}
...
\centerline{\includegraphics[width=0.47\textwidth]{figs/11-bronchitAB-1} }
...
\centerline{\includegraphics[width=0.98\textwidth]{figs/12-ant111b-1} }
...
\marginnote{Smoothing terms can be fitted to the pattern apparent in
  ...
  It gives little clue of what may happen the future.}
...
from 1918 to 2009, will be used as a basis for discussion.\sidenote{Data are
  from {\scriptsize
...
assumes\marginnote{The fitted model is conditional on the particular
  realization of the time series process.}  independently and
...
The following code fits from two slightly different GAM smoothing
models, leading to the plots shown in Figure \ref{fig:planeCrash}:
\begin{fullwidth}
...
\centerline{\includegraphics[width=0.97\textwidth]{figs/12-plotGAM-1} }
...
\marginnote{Technicalities of logistic regression, using, e.g.,
  R's function \txtt{glm()}:
  \begin{itemizz}
\item[-] The fitting procedure minimizes the {\em deviance}. This
  equals 2 ( {\em loglikelihood} for fitted model - loglikelihood for
  `saturated' model). The `saturated model has predicted values
equal to observed values.
...
\item[-] Logistic regression is a special case of a Generalized Linear
  Model (GLM).
\end{itemizz}
}
...
\end{Soutput}
\end{Schunk}
Here \txtt{r=1} denotes bronchitis,  while \txtt{r=0}
indicates that bronchitis is absent.
In place of the variable \txtt{r} with values 0 and 1, we use a
factor with levels \txtt{abs} and \txtt{pres} that will be used
...
\txtt{method="class"} is the default.}
...
\end{Schunk}
\noindent
Note the desirability of using a splitting rule, here \txtt{"class"}
...
\centerline{\includegraphics[width=0.98\textwidth]{figs/12-proximity-plot-1} }
