...
\begin{marginfigure}
<<oz-sites, echo=FALSE, w=4, h=6.5, rt=3,crop=TRUE, out.width="\\textwidth">>=
@ 
\vspace*{3pt}
...
Another possibility is to use the function \texttt{spplot()}
  ...
  \ref{fig:rlogo3}:
\begin{figure}
<<spplot-col3, w=6, h=2, echo=FALSE, out.width="0.8\\textwidth">>=
@ %
\caption{Red, green and blue layers from the R logo image.\label{fig:rlogo3}}
\end{figure}
<<spplot-col3, eval=FALSE>>=
@ %
...
before inputting it:
<<sp27-info, eval=FALSE>>=
@ %
\end{marginfigure}
<<sp27-read, eval=TRUE>>=
@ %
Then to plot the image, enter:
<<sp27-spplot, eval=FALSE>>=
@ %

\subsection{Overlaying onto a bubble plot}

\begin{marginfigure}
<<meuse-bubble, echo=FALSE, w=3.75, h=4, crop=TRUE, out.width="0.98\\textwidth">>=
@ %
\vspace*{3pt}
...
<<load-latticeExtra, eval=FALSE>>=
@ %

<<meuse-bubble, eval=FALSE>>=
@ %
