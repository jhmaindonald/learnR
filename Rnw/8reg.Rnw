...
opts_chunk$set(fig.path='figs/8-', cache.path='cache/8-',
               fig.align='center', dev='pdf',  
               fig.show='hold', out.width="0.47\\textwidth", 
               tidy=FALSE, comment=NA)
pdf.options(pointsize=12)
oldopt <- options(digits=4)
read_chunk("../scripts/reg-code.R")
@ 

\fbox{\parbox{\textwidth}{{\bf Linear Models, in the style of \code{lm()}:}\\[4pt]
\begin{tabular}{ll}
Linear model & Any model that \code{lm()} will fit
is a `linear' model.\\
& \code{lm()} can fit highly non-linear forms of response!\\[6pt]
Diagnostic  & Use \code{plot()} with the model object as argument,\\
plots & to get a basic set of diagnostic plots.\\[6pt]
\code{termplot()} & If there are no interaction terms,
use \code{termplot()}\\
...
regression & of explanatory variables that adapt to suit the data.\\[6pt]
GAMs & Generalized Additive Models extend linear models\\
& to allow smoothing curves and surfaces.\\[6pt]
\end{tabular}
\vspace*{3pt}

[NB: \code{lm()} assumes independently \& identically distributed (iid)\\
errors, perhaps after applying a weighting function.]
}}
\vspace{12pt}

In this chapter, the chief focus will be on the \code{lm()}
(\textit{linear model}) function, discussed earlier in Section
\ref{sec:wr}.  The \code{lm()} function is the most widely used of a
...
that can be fitted using R's linear model \code{lm()} function.

The \code{lm()} function returns, as well as estimates, standard
errors for parameters and for predictions. The standard error and
$p$-value information provided by the \code{lm()} function assumes
...
The following plots data from the data frame \code{DAAG::roller} 
(as in Figure \ref{fig:rollerPlot}).
<<plt-roller, eval=FALSE>>=
@ %

\begin{marginfigure}
<<plt-roller, echo=FALSE, w=3.5, h=3, crop=TRUE, out.width="\\textwidth">>=
@ %
  \caption{Plot of \code{depression} versus \code{weight}, using data
from the data frame \code{DAAG::roller}.}\label{fig:rollerPlot}
\end{marginfigure}
...
\begin{marginfigure}
<<pltWline, echo=FALSE, w=3.5, h=3, crop=TRUE, out.width="\\textwidth">>=
@ %
...
`terms'.  In the above, there is one explicit term only on the
right, i.e., \code{weight}. This is in addition to the intercept,
...
\vspace*{-4pt}
\]
Now write $y$ in place of \code{depression} and $x$ in place of
\code{weight}, and add subscripts, so that the observations are:
...
The syntax for \code{lm()} models that will be demonstrated here is
...
\marginnote{Components of model objects can be accessed directly, as
  ...
  function. Note in particular \code{residuals()} (can be
  abbreviated to \code{resid()}), \code{coefficients()}
  (\code{coef()}), and
  \code{fitted.values()} (\code{fitted()}).  For example:\\
  \code{coef(roller.lm)} }
...
The matrices and vectors required for the matrix calculations are:
\begin{fullwidth}
...
The SEs returned by \code{lm()} or by \code{predict()} from
an \code{lm} model will be problematic or wrong if:
\begin{itemize}
...
affect coefficients that remain in the model, may be acceptable.}
 ...
 theory cannot be satisfactorily used as a good approximation.
...
\begin{figure}
<<fVSmTimeAB, echo=FALSE, w=5.5, h=3.05, bot=2, top=1, out.width="\\textwidth">>=
@ %
...
<<coef-logmf>>=
@ %
The coefficient of 1.042 for \code{log(time)} implies that the
...
\begin{figure}
<<tohoriz-mf, echo=FALSE, mfrow=c(1,2), w=8, h=3.35, mgp=c(2.5,0.5,0), top=2, rt=1.6, bot=0.5, left=1, out.width="\\textwidth">>=
@ %
...
\marginnote{A residual of -0.1 denotes a
...
\begin{itemize}
  ...
  \item[-] Working on a logarithmic scale is equivalent to working
    ...
    $\log(2)$.
  \item[-] By default, the function \code{log()}) returns natural
...
\begin{figure*}
<<diag-mf, echo=FALSE, mfrow=c(1,4), w=8, h=2, top=1.5,  right=1, ps=11, out.width="\\textwidth">>=
@ %
\caption{Diagnostic plots from the regression of \code{timef} on
  \code{time}.}\label{fig:diag-mftime}
\end{figure*}
\noindent
Simplified code is:
<<diag-mf, eval=FALSE, echo=2:3>>=
@ %
...
Simplified code is:
<<diag-logmf, eval=FALSE, echo=2:3>>=
@ %
\begin{figure*}
<<diag-logmf, echo=FALSE, mfrow=c(1,4), w=8, h=2, top=1.5, ps=11, right=1, out.width="\\textwidth">>=
@ %
\caption{Diagnostic plots from the regression of \code{log(timef)} on
  \code{log(time)}.}\label{fig:diag-mftime-log}
\end{figure*}
\noindent
...
that residuals have a normal distribution.  Again, Figure
...
\marginnote{If `Seven Sevens' is omitted in estimating the
...
<<simscat, echo=FALSE, w=7, h=3, out.width="\\textwidth">>=
@ %
...
\begin{itemize}
  \item [-] A check for normality (Panel 2: \code{which=2}.  Type:
<<simdiag2, eval=FALSE>>=
@ %
\item[-] Is the variance constant? (Panel 3: \code{which=3}.  Type:
<<simdiag3, eval=FALSE>>=
@ %
...
use the function \code{plotsimscat()}, from the \pkg{DAAG} package.
For example, try, working with the untransformed data:
<<mftime-sims, eval=FALSE>>=
@ %
...
An algorithm is not on its own enough, if results are to have
a relevance that generalizes beyond the specific data used.
...
in Subsection \ref{sec:nihills-reg}, the parameters \code{dist} and
\code{climb} clearly do not exercise their effects independently,
making their coefficients difficult to interpret. It is better to work
with \code{log(dist)} and \code{log(dist/climb)}, which are very
nearly independent.
...
\end{tabular}
\caption{Root weights (\code{weight}) (g) of tomato
  plants, grown with water only and grown wth three different
  treatments. Data are in the data frame \code{DAAG::tomato}.\label{tab:tomatowt}}
\end{table}
\vspace*{9pt}

Additive A is \code{conc nutrient}, B is \code{3x conc nutrient}, and
C is \code{2-4-D + conc nutrient}.  For convenience, we label the factor levels
\code{Water}, \code{A}, \code{B}, and \code{C}, in that order.
<<lev-tomato>>=
@ %
Taking \code{Water} as the initial level the effect, in the first
analysis that is given below, that it is treated as a reference
level.

\subsection{Example -- tomato root weight}\label{tomato}

The model can be fitted either using the function \code{lm()} or
using the function \code{aov()}.  The two functions give different
default output.  The main part of the calculations is the same
whether \code{lm()} or \code{aov()} is used.
...
The following uses \code{aov()} for the calculations:
<<tomato-aov>>=
@ %
...
\begin{figure}
<<termplot-aovAB, echo=FALSE, mfrow=c(1,2), top=1, w=8, h=3.45, out.width="\\textwidth">>=
<<termplot-aov-wt>>
<<termplot-aov-logwt>>
@ %
...
B: for the analysis that works with \code{log(weight)}}
\label{fig:tomatoterm}
\end{figure}

\noindent
Code for Figures \ref{fig:tomatoterm}A and \ref{fig:tomatoterm}B is:
<<termplot-aovAB, eval=FALSE, echo=-c(5,11)>>=
@ %
...
Coefficient estimates for the model that uses \code{weight} as the
...
value, which can be calculated either as \code{fitted(tomato.aov)} or as
\code{predict(tomato.aov)}.
\begin{table}
\tabcolsep9pt
...
`contrasts' are available, with each different choice leading
to a different model matrix and to different regression parameters.
The fitted values remain the same, the termplot in Figure 
{fig:tomatoterm}A
...
The data frame \code{cuckoos} has the lengths and breadths of cuckoo
...
\noindent Here, the model \code{cuckoos.lm}, where the regression lines are
...
<<anova-Cp, eval=FALSE>>=
@ %
The `Cp' statistic (smaller is better) compares models on the basis
of an assessment of their predictive power. Note the use of the
argument \code{test="cp"}, even though this is not a comparison
...
The dataset \code{DAAG::nihills} has
...
\begin{figure*}
\vspace*{-6pt}
<<splot2-ni, echo=FALSE, w=5.25, h=5.25, out.width="0.48\\textwidth">>=
<<scatter-ni>>
<<scatter-logni>>
@ %
...
\noindent
Thus for constant \code{climb}, the prediction is that time per mile
...
\code{log(time)} on \code{log(dist)} and \code{log(gradient)},
where \code{gradient} is \code{dist/climb}.
<<nireg-slope>>=
@ %
Thus, with \code{gradient} held constant, the prediction is that \code{time}
will increase at the rate of $\mbox{dist}^{1.147}$.  This makes good intuitive
sense.

\begin{figure}
<<tplot-ni, echo=FALSE, mfrow=c(1,2), w=8, h=3.35,bot=0.5, left=1, out.width="\\textwidth">>=
@ %
\caption{The vertical scales in both ``term plot''  panel
  show $\log(\rm{time})$, centered to a mean of zero. The partial residuals
  in the left panel are for \code{ldist}, while those in the right
  panel are for \code{lgradient}, i.e.,
  \code{log(climb/dist)}. Smooth curves (dashes) have been passed
  through the points.\label{fig:lnihills-lin}}
\vspace*{-5pt}
\end{figure}

We pause to look more closely at the model that has been fitted.  Does
\code{log(time)} really depend linearly on the terms \code{ldist} and
\code{log(lclimb)}?  The function \code{termplot()} gives a good
graphical indication (Figure \ref{fig:lnihills-lin}).
<<tplot-ni, eval=FALSE>>=
@ %

The vertical scales show changes in \code{ltime}, about the mean of
\code{ltime}.  The lines show the estimated effect of each explanatory
...
Notice that, in the plot for \code{ldist}, the smooth dashed line does
...
The function \code{bestsetNoise()} (\pkg{DAAG}) can be used to
...
Section 6.5, pp.~197-198, in \citet{daagur_2011}.}

<<varselect-sim, eval=FALSE>>=
@ %
...
<<bsnVary, echo=FALSE, w=3.5, h=3.15, top=1, crop=TRUE, out.width="\\textwidth">>=
@ %
...
<<Elec-spm, echo=FALSE, w=8, h=8, out.width="0.775\\textwidth">>=
@ %
}
...
\caption{Scatterplot matrix, for the variables in the data set
  \code{Ecdat::Electricity}. Density
...
is:
 ...
 \item[(i)] Start with a model that includes all plausible main effects
   (variables and factors).  Ensure that the model is parameterized in
   a way that makes parameters of interest as far as possible
   interpretable (e.g., in Subsection \ref{sec:nihills} above, work
   with \code{distance} and \code{gradient}, not \code{distance}
   and \code{climb})
 \item[(ii)]
 ...
 \item[(iii)] Variables and/or factors that have no detectable main
   ...
   model that includes all 2-way interactions. Then, using $p$ $\simeq$
 ...
 \item[(iv)] In principle, the process may be repeated for order 3
   interactions.
\item[(v)] Use the function \code{add1()} to check for individual
...
The distributions of \code{cost} and \code{q} are highly skew.
...
logarithms of the variables \code{cost} and \code{q}.
\noindent
Code is:
\begin{marginfigure}[-3.5cm]
<<spm-cost-q, echo=FALSE, w=6, h=6, out.width="\\textwidth">>=
@
  \caption{Scatterplot matrix for the logarithms of the variables
    \code{cost} and \code{q}. Density plots are shown in the
    diagonal.\label{fig:elec-dens}}
\end{marginfigure}
<<spm-cost-q, eval=FALSE>>=
@ %
...
\begin{figure}[h]
<<elec-me-tplot, echo=FALSE, mfrow=c(2,4), w=8, h=3, out.width="\\textwidth">>=
@ %
\caption{Termplot summary for the model that has been fitted to the
  \code{Electricity} dataset.\label{fig:elec-log-tplot}}
\end{figure}
\noindent Code is:
<<elec-me-tplot, eval=FALSE>>=
@ %
Notice that in the partial plot for \code{q}, the dashed curve that is
fitted to the residuals closely tracks the fitted effect (linear on a
scale of \code{log(q)}.  This confirms the use of \code{log(q)},
rather than \code{q}, as explanatory variable.

Now examine the model output:
<<elec-me-coef>>=
@ %

The $p$-values suggest that \code{pk}, \code{sk}, and \code{sf}
...
<<coef-elecx-sum>>=
@ %
This suggests omitting the terms \code{pf}, and all interactions
except \code{log(q):pl} and \code{log(q):sl}.  We check that omission
...
\marginnote[11pt]{The dataset \margtt{bronchit} may alternatively be 
 found in the \pkg{SMIR} package.}

<<load-DAAGviz-KernSmooth, echo=FALSE>>=
@ %

<<get-bronchit, echo=FALSE>>=
@

The data that will be used for illustration are from the data frame
\code{DAAGviz::bronchit}.  Figure \ref{fig:cig-poll} 
shows two plots -- one of \code{poll}
(pollution level) against \code{cig} (number of cigarettes per day),
and the other of \code{poll} against \code{log(poll)}.  In each
case, points are identified as with or without bronchitis.

\begin{figure*}
<<bronchitAB, echo=FALSE, w=8, h=3.35, ps=11, mfrow=c(1,2), crop=TRUE, out.width="0.95\\textwidth">>=
<<get-bronchit>>
<<bronchitA>>
<<bronchitB>>
@ %
\caption{Panel A plots \code{poll} (pollution level) against
  \code{cig} (number of cigarettes per day).  In panel B, the
...
The following loads packages that will be needed:

<<load-DAAGviz-KernSmooth, eval=FALSE>>=
@ %

<<bronchitA, eval=FALSE, echo=1:9>>=
@ %

%$

\begin{fullwidth}
<<bronchitB, eval=FALSE, echo=1:11>>=
@ %
\end{fullwidth}
...
\begin{figure}[h]
<<cig2-tplot, echo=FALSE, mfrow=c(1,2), w=8, h=3.5, out.width="\\textwidth">>=
@ %
...
Code is:
<<cig2-tplot, eval=FALSE>>=
@ %

\section{Regression with Fitted Smooth Curves}

Load the \pkg{DAAG} package:
<<loadDAAG>>=
@ %
...
\code{lowess()}, \code{loess()}, \code{loess.smooth()}, 
and \code{scatter.smooth()}.  Smoothness
is controlled by the width of the smoothing window.\sidenote{The 
default is \code{f=2/3} for \code{lowess()}, or \code{span=0.75} 
for \code{loess()}. For other functions that use this approach,
...
\item A second class of methods use a ``statistical learning''
  ...
  The approach of the \pkg{mgcv} package extends and adapts the
...
\begin{marginfigure}
<<smooth-ohms, echo=FALSE, w=3.25, h=3, ps=11, crop=TRUE, out.width="0.98\\textwidth">>=
@ %
...
<<smooth-ohms, eval=FALSE>>=
@ %

A more sophisticated approach uses the \code{gam()} function
\marginnote{\Citet{Wood-2017} is a comprehensive account of GAM
models that documents the abilities of the \pkg{mgcv} package.}
in the \pkg{mgcv} package.  This allows automatic determination of the
...
function \code{gam()} to create smooth
surfaces, for males and females separately. Figure \ref{fig:visAmp}
then uses the function \code{vis.gam()} to plot heatmaps that show the
contours:
\begin{fullwidth}
\begin{figure*}
<<plotVIS, w=8.5, h=3.5, mfrow=c(1,2), top=2, ps=11, out.width="0.97\\textwidth", echo=FALSE>>=
@ %
\vspace*{-7pt}
...
groups, a side effect of the attenuation is to make it appear that the
...
Examples on the help page \code{?DAAG::errorsINx} show how to run
simulations that demonstrate the effects in the simple cases just
described.  These are useful in giving a sense of the magnitude
...
Control data.  \Citet{lazerEtAl_GFT_2014} warn against what
...
subtleties.  Image recognition software, using convolutional  neural networks (CNNs), can be readily fooled by `adversarial' images.
Thus, minor doctoring of an image of an orange may lead to its
...
mechanisms that the human brain uses.  Wider experience of the 
outside world may in part explain the difference.

\section{References and further reading}

Chapter 1 of \citet{Wood-2017}  is a good brief introduction to
linear models.  Chapter 6 of \citet{daagur_2011} is a more elementary
and more extended introduction.

\section{Exercises}
\begin{enumerate}
\item Exercise \ref{ex:mol1} in Section \ref{ss:wd} involved reading
  data into a data frame \code{molclock1}.  Plot \code{AvRate} against
  \code{Myr}. Fit a regression line (with intercept, or without
  intercept?), and add the regression line to the plot.  What
  interpretation can be placed upon the regression
  slope?\label{ex1:mod}

\item  Attach the \pkg{DAAG} package. Type
\code{help(elasticband)} to see the help page for the data frame
\code{elasticband}.  Plot \code{distance} against
\code{stretch}. Regress \code{distance} against
\code{stretch} and explain how to interpret the coefficient.
\item Repeat the calculations in Section \ref{sec:nihills-reg}, now
examining the regression of \code{time} on \code{dist} and \code{climb}.
...
the standard errors.
\item Can \code{weight} be completely explained as a function of
\code{volume}?  Is there another relevant variable?
\end{enumerate}
\item Repeat the calculations in Section \ref{sec:nihills}, now
  with the dataset \code{DAAG::hills2000}.  Do any of
  the points stand out as outliers?  Use \code{predict()},
  with \code{newdata = hills200}, to obtain predictions from the
  \code{hills2000} model for the \code{nihills} data.  Compare with
  the predictions from the \code{nihills} model.
\end{enumerate}
