...
\txtt{bronchit} in the {\em DAAGviz} package.  Figure \ref{fig:cig-poll} 
...
\txtt{lowess()}, \txtt{loess()}, \txtt{loess.smooth()}, 
and \txtt{scatter.smooth()}.  Smoothness
is controlled by the width of the smoothing window.\sidenote{The 
default is \txtt{f=2/3} for \txtt{lowess()}, or \txtt{span=0.75} 
for \txtt{loess()}. For other functions that use this approach,
